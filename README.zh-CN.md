...
- **搜索无需选择匹配模式。** 明细与全局搜索直接输入关键词即可自动查询，保留日期、金额、分类、钱包及账本筛选；全局搜索默认覆盖全部可访问账本。这两个入口目前**不是向量搜索**。向量检索用于商品记忆匹配和助手记忆查询，嵌入服务不可用时退回关键词检索。
- **页内记忆管理。** 在 **个人资料 → 记忆中心** 管理记忆、待确认建议、变更记录和学习设置。顶层导航、分区切换与操作按钮有不同样式；变更记录展示当前对象名称、操作、时间及撤销状态，支持分页和手机布局，不重新展示已遗忘的内容。
- **任意统计周期与完整详情。** 可按年、月、周、日或指定日期范围统计；每月预算仍按月管理。从图表进入账单详情可查看交易与钱包信息、订单／流水号、商品与优惠费用、备注和已保存附图。
- **助手管理确认流程。** 支持的系统操作通过权限校验后的确认卡或对应页面入口完成，追问沿用仍有权限的账本范围与已有查询依据。具体边界见[助手能力说明](docs/assistant-capability-audit.md)；密码、密钥与上传仍使用专门界面。
- **日期与时间分开。** 交易日期固定为 `YYYY-MM-DD`，缺失时使用记录日期；交易时间为独立的规范字段，未知时可留空。优惠、满减和手续费归入独立结算区域，不再作为商品参与记忆匹配。
...
| 时间与备注 | 交易日期缺失时使用记录日期，未知交易时间保持留空；创建时间和修改时间单独记录。备注聚焦用途等有价值信息。 |
...
首页的 **进行中的活动 → 记一笔到活动** 可直接进入记账；手动记账、识图草稿或活动页的 **加入已有交易** 也能指定活动。助手可以查找活动、准备带活动归属的交易确认卡，以及受支持的活动管理操作；变更仍需用户确认并通过权限校验。活动详情提供分类、付款钱包、账本与每日支出图表，以及可搜索、筛选和分页的交易明细。
