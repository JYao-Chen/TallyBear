...
当前开发预览已将新复购关联接入统一记忆中心；本节下文描述的 `purchaseGroup` 同款汇总属于旧数据路径。新关联可在个人资料／助手 → 记忆中心查看来源与购买次数，模型配置、开发范围和发布验收状态见[财务记忆说明](financial-memory.md)。旧分类加权保留，新商品匹配不再限于最近 2,000 条交易，但需先完成历史整理。生产是否可用以实际发布版本为准。
