...
try{
 ...
 const edit={operation:'edit',id:inboxId,displayBookId:pa,title:'晚餐红包',note:'自己的记录',transactionTime:'20:35',platform:'微信',externalId:'proof-123'};
 await api(a,f,'POST',edit);
 const own=(await api(a,f,'GET',{}) as any).movements.find((m:any)=>m.id===inboxId);
 assert.equal(own.details.title,'晚餐红包');assert.equal(own.details.transaction_time,'20:35');
 assert.equal((await api(b,f,'GET',{}) as any).movements.find((m:any)=>m.id===inboxId).details,null);
 await assert.rejects(()=>api(outsider,f,'POST',edit));
 await assert.rejects(()=>api(a,f,'POST',{...edit,transactionTime:'25:61'}));
 const attachment=randomUUID();await db.query("INSERT INTO receipt_files(id,user_id,name,mime,movement_id,movement_purpose) VALUES($1,$2,'proof','image/webp',$3,'receipt')",[attachment,a,inboxId]);
 assert.equal((await api(a,f,'GET',{}) as any).movements.find((m:any)=>m.id===inboxId).attachments.length,1);
 assert.equal((await api(b,f,'GET',{}) as any).movements.find((m:any)=>m.id===inboxId).attachments.length,0);
 await api(b,f,'POST',{...edit,displayBookId:pb,removeAttachments:[attachment]});
 assert.equal((await db.query('SELECT movement_id FROM receipt_files WHERE id=$1',[attachment])).rows[0].movement_id,inboxId);
 await api(a,f,'POST',{...edit,removeAttachments:[attachment]});
 assert.equal((await db.query('SELECT temporary FROM receipt_files WHERE id=$1',[attachment])).rows[0].temporary,true);
 assert.ok((await exportCSV(pa,august)).includes('晚餐红包'));
 assert.equal((await ledgerReport(pa,august)).totals.expense,222);
 console.log('PASS editable private movement details, attachment ownership/removal, unchanged financial totals');
