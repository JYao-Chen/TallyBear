...
import {listAccounts,changeAccount,validateActiveAccounts,accountMetadata,accountReport} from '@/server/accounts';
...
async function handle(req:NextRequest,ctx:Ctx){const locale=deployment().language;const tr=(s:string)=>translate(s,locale);try{
 ...
 if(path[0]==='wallets')return NextResponse.json(await wallets(u.id,method,body));
 ...
 if(resource==='account-report'&&method==='GET')return NextResponse.json(await accountReport(book,req.nextUrl.searchParams));
 if(resource==='account-history'&&method==='GET'){const account=uuid.parse(req.nextUrl.searchParams.get('account'));return NextResponse.json((await db.query('SELECT j.id,j.amount::float8 AS amount,j.note,j.created_at,u.name AS creator FROM account_adjustments j JOIN users u ON u.id=j.created_by WHERE j.book_id=$1 AND j.account_id=$2 ORDER BY j.created_at DESC LIMIT 100',[book,account])).rows);}
 if(resource==='accounts'){
  if(method==='GET')return NextResponse.json(await listAccounts(book));
  if(method==='PATCH')return NextResponse.json(await changeAccount(book,u.id,body));
  if(method==='POST'){const b=z.object({name,opening:z.number().int().min(-100000000000).max(100000000000).default(0),...accountMetadata}).parse(body);await db.query('INSERT INTO accounts(id,book_id,name,opening,type,holder,institution,suffix,ownership) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)',[randomUUID(),book,b.name,b.opening,b.type||'other',b.holder||'',b.institution||'',b.suffix||'',b.ownership||'unspecified']);return NextResponse.json({ok:true});}
 }
   ...
   await transaction(async c=>{await lockBook(c,book);
    for(const raw of [...items].sort((a,b)=>Number(a.kind==='refund')-Number(b.kind==='refund'))){
     ...
     if(raw.action==='merge'){
      ...
      const result=await c.query("UPDATE transactions SET product=$1,order_id=$2,platform=$3,occurred_at=$4,note=$5,payee=$6,category=$7,line_items=$10::jsonb,version=version+1 WHERE book_id=$8 AND id=$9",[old.product||d.product,old.order_id||d.orderId,old.platform||d.platform,old.occurred_at||d.occurredAt,old.note||d.note,old.payee||d.payee,old.category==='其他'?d.category:old.category,book,raw.mergeId,JSON.stringify(old.line_items?.length?old.line_items:d.lineItems)]);merged+=result.rowCount||0;if(old.kind==='expense')await c.query('UPDATE transactions SET category=$1,version=version+1 WHERE book_id=$2 AND refund_of=$3 AND NOT deleted',[old.category==='其他'?d.category:old.category,book,raw.mergeId]);continue;
     }
     const e=entry.parse(raw);added+=await insertEntry(c,book,u.id,e);
    }
   });return NextResponse.json({added,merged,skipped:items.length-added-merged});
  }
  if(method==='PUT'){const e=entry.parse(body);checkVerification(e.lineItems,e.amount,e.verificationReason);const version=z.number().int().parse(body.version);const result=await transaction(async c=>{await lockRelatedBooks(c,book,e.id,u.id);const previous=(await c.query('SELECT account_id,target_id,category,line_items,verification_reason FROM transactions WHERE book_id=$1 AND id=$2',[book,e.id])).rows[0];if(body.lineItems===undefined&&previous)e.lineItems=previous.line_items;if(body.verificationReason===undefined&&previous)e.verificationReason=previous.verification_reason;checkVerification(e.lineItems,e.amount,e.verificationReason);await validateActiveAccounts(c,book,e,previous?[previous.account_id,previous.target_id].filter(Boolean):[]);await validateRefund(c,book,e);await checkCategory(c,book,e.category,previous?.category);await syncFinancialFacts(c,book,e,u.id);const r=await c.query('UPDATE transactions SET account_id=$1,target_id=$2,kind=$3,amount=$4,date=$5,payee=$6,category=$7,note=$8,product=$12,platform=$13,order_id=$14,occurred_at=$15,refund_of=$16,line_items=COALESCE($17::jsonb,line_items),version=version+1 WHERE id=$9 AND book_id=$10 AND version=$11 AND NOT deleted RETURNING version',[e.accountId,e.kind==='transfer'?e.targetId:null,e.kind,e.amount,e.date,e.payee,e.category,e.note,e.id,book,version,e.product,e.platform,e.orderId,e.occurredAt,e.refundOf||null,body.lineItems===undefined?null:JSON.stringify(e.lineItems)]);if(!r.rowCount)throw new Failure('记录已被其他成员修改，请刷新后再试',409);await c.query('UPDATE transactions SET verification_reason=$1,title=COALESCE($3,title) WHERE id=$2',[e.verificationReason,e.id,body.title===undefined?null:e.title]);await linkReceipts(c,book,u.id,e.id,e.attachmentIds,e.retainReceipts);await linkReceipts(c,book,u.id,e.id,e.photoIds,true,'photo');if(e.kind==='expense')await c.query('UPDATE transactions SET category=$1,version=version+1 WHERE book_id=$2 AND refund_of=$3 AND NOT deleted',[e.category,book,e.id]);return r.rows[0];});return NextResponse.json(result);}
  if(method==='DELETE'||method==='PATCH'){const b=z.object({id:uuid,version:z.number().int(),deleted:z.boolean().optional()}).parse(body);const deleted=method==='DELETE'?true:b.deleted??false;const result=await transaction(async c=>{await lockBook(c,book);if(deleted){const links=await c.query('SELECT id FROM transactions WHERE book_id=$1 AND refund_of=$2 AND NOT deleted',[book,b.id]);if(links.rowCount)throw new Failure('原消费关联了退款，请先处理退款记录');}else{const row=(await c.query(`SELECT *,account_id AS "accountId",target_id AS "targetId",refund_of AS "refundOf",amount::float8 AS amount,to_char(date,'YYYY-MM-DD') AS date FROM transactions WHERE book_id=$1 AND id=$2`,[book,b.id])).rows[0];if(row)await validateRefund(c,book,row);}
