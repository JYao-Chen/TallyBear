'use client';
import {FamilyActionEditor} from './FamilyActionEditor';
...
function StandardActionEditor({action,onSave,onCancel}:{action:ChatAction;onSave:(data:Record<string,any>)=>Promise<void>;onCancel:()=>void}){
 ...
 const keys=({entry:['title','amount','date','occurredAt','payee','note'],transaction:['title','amount','date','occurredAt','payee','note'],template:['name','title','amount','note'],schedule:['name','amount','nextDate','intervalCount','note'],budget:['amount','month'],allocation:['startDate','periodCount'],installment:['name','amount','date','terms','firstDate','fees'],repayment:['date','principal','fee']} as const)[action.kind as Exclude<ChatAction['kind'],'family'>];
 ...
 {options&&['entry','transaction'].includes(action.kind)&&<label>{t('所属活动（选填）')}<VisualSelect label={t('所属活动')} value={data.activityId||''} onChange={v=>update('activityId',v||null)} options={[{value:'',label:t('不归入活动')},...options.activities.map((a:any)=>({value:a.id,label:a.name}))]}/></label>}
 {['entry','transaction','template','schedule'].includes(action.kind)&&<SceneFields value={data.scene} onChange={(scene,title)=>setData(d=>({...d,scene,...(title?{title}:{})}))}/>}<div className="form-grid">{keys.map(key=><label key={key}>{t(fields[key])}<input name={key} value={['amount','principal','fee','fees'].includes(key)?data[key+'Input']??(data[key]==null?'':String(data[key]/100)):data[key]??''} onChange={e=>update(['amount','principal','fee','fees'].includes(key)?key+'Input':key,e.target.value)} inputMode={['amount','principal','fee','fees'].includes(key)?'decimal':undefined}/></label>)}</div>
 {['entry','transaction','template','schedule'].includes(action.kind)&&<LineItemsEditor items={lineItemsSchema.safeParse(data.lineItems).success?lineItemsSchema.parse(data.lineItems):data.lineItems||[]} total={data.amountInput!==undefined?Number(data.amountInput)*100:data.amount||0} onChange={items=>update('lineItems',items)} onTotal={amount=>setData(d=>({...d,amount,amountInput:String(amount/100)}))}/>} {error&&<p className="error" role="alert">{error}</p>}<div className="inline"><button disabled={busy||!options}>{t('更新卡片')}</button><button type="button" className="secondary" disabled={busy} onClick={onCancel}>{t('取消修改')}</button></div>
 </form>;
}

export function ChatActionEditor(props:Parameters<typeof StandardActionEditor>[0]){return props.action.kind==='family'?<FamilyActionEditor {...props}/>:<StandardActionEditor {...props}/>;}
