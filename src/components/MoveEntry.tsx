...
export function MoveEntry({book,id,books,onSaved}:{book:string;id:string;books:ManagedBook[];onSaved:()=>void}){
 const {t:tr}=useI18n();const [open,setOpen]=useState(false),[preview,setPreview]=useState<Preview|null>(null),[target,setTarget]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState('');
