...
export function Sheet({title,onClose,children,variant='detail'}:{title:string;onClose:()=>void;children:ReactNode;variant?:'detail'|'picker'|'wide'}){const {t:tr,locale}=useI18n();
 ...
 useEffect(()=>{if(!mounted)return;const d=dialog.current!,previous=document.activeElement as HTMLElement|null;d.showModal();if(variant==='picker')d.querySelector<HTMLElement>('.choice-search input, .choice-option[aria-pressed="true"], .calendar-days button[aria-pressed="true"], .calendar-months button[aria-pressed="true"]')?.focus({preventScroll:true});return()=>{d.close();if(previous?.isConnected)previous.focus({preventScroll:true});};},[mounted,variant]);
