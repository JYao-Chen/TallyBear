...
import {lineItemKind,settlementNameKind} from './line-items';
...
export function memoryProducts(input:ProductInput){
 const goods=(input.lineItems||[]).filter(i=>lineItemKind(i)==='item'&&!settlementNameKind(i.name)&&i.name.trim());
 ...
 if(!name||settlementNameKind(name)||[input.payee,input.category,'支出','消费','购物','其他'].some(v=>v&&normMemory(v)===normMemory(name)))return [];
