...
export function matchReceiptWallet(origin:ReceiptOrigin|undefined,wallets:Wallet[],user:string){
 const f=origin?.funding;if(!f||f.type==='unknown'||!f.evidence.trim())return {accountId:'',candidates:[] as string[]};
 ...
 if(['bank','credit'].includes(f.type)&&!f.suffix&&!f.institution)return {accountId:'',candidates:candidates.map(a=>a.id)};
 return {accountId:candidates.length===1?candidates[0].id:'',candidates:candidates.map(a=>a.id)};
}
