...
export async function accountReport(user:string,params:URLSearchParams){
 ...
 const accounts=selectPersonalAssets(await listAssets(user),user);const ids=accounts.map(a=>a.id);
 ...
 const movements=(await db.query("SELECT v.source_id,v.target_id,v.kind,v.amount::float8 AS amount,s.name AS sender_name,r.name AS recipient_name FROM family_movements v JOIN users s ON s.id=v.sender_id LEFT JOIN users r ON r.id=v.recipient_id WHERE v.status='confirmed' AND (v.source_id=ANY($1::uuid[]) OR v.target_id=ANY($1::uuid[])) AND v.date BETWEEN $2::date AND $3::date",[ids,p.from,p.to])).rows;
 for(const m of movements){if(m.kind==='gift'){const outgoing=ids.includes(m.source_id);flows.push({account_id:outgoing?m.source_id:m.target_id,target_id:null,kind:outgoing?'expense':'income',amount:m.amount,category:'人情往来',payee:outgoing?m.recipient_name:m.sender_name});}else flows.push({account_id:m.source_id,target_id:m.target_id,kind:'transfer',amount:m.amount,category:'',payee:''});}
 ...
 return {period:p,scope:'personal',accounts,sources:[...sourceMap.values()].sort((a,b)=>b.amount-a.amount),ownershipGroups:[{ownership:'personal',count:accounts.length,netAssets:summary.netAssets,expense:summary.expense-summary.refund}],summary};
}
