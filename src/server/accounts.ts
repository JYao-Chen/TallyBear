...
export async function accountReport(user:string,params:URLSearchParams){
 ...
 flows.push(...(await db.query("SELECT source_id AS account_id,target_id,'transfer' AS kind,amount::float8 AS amount,'' AS category,'' AS payee FROM family_movements WHERE status='confirmed' AND (source_id=ANY($1::uuid[]) OR target_id=ANY($1::uuid[])) AND date BETWEEN $2::date AND $3::date",[ids,p.from,p.to])).rows);
