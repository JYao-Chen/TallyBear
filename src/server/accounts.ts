...
export async function accountReport(user:string,params:URLSearchParams){
 ...
 for(const m of movements){if(scope==='personal'&&m.kind==='gift'){const outgoing=ids.includes(m.source_id);flows.push({account_id:outgoing?m.source_id:m.target_id,target_id:null,kind:outgoing?'expense':'income',amount:m.amount,category:'人情往来',payee:outgoing?m.recipient_name:m.sender_name});}else flows.push({account_id:m.source_id,target_id:m.target_id,kind:'transfer',amount:m.amount,category:'',payee:''});}
