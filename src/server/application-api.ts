...
import {copyEntryReceipts,moveEntry,movePreview,moveInTransaction,selectedGroup,withReceiptTransaction} from '@/server/move-entry';
...
import {recordCategoryFeedback,persistMemoryFeedback} from '@/server/receipt-preferences';
...
export async function applicationApi(req:NextRequest,ctx:Ctx,actor?:User){const locale=deployment().language;const tr=(s:string)=>translate(s,locale);try{
 ...
 if(path[0]==='receipts')return await receiptRoute(req,path,(actor??await user()));
 const photoRequest=path[0]==='books'&&path.length===3&&['assistant','chat','drafts'].includes(path[2]);
 if(photoRequest&&method!=='GET')(actor??await user());
 ...
 if(path[0]==='activities'){
  ...
  if(path[1]&&method==='GET')return NextResponse.json(await activityReport(u.id,path[1],req.nextUrl.searchParams));
  if(method==='GET')return NextResponse.json(await listActivities(u.id));
  return NextResponse.json(await changeActivity(u.id,method,body));
 }
 ...
 if(resource==='refund-options'&&method==='GET'){
  const params=new URLSearchParams(req.nextUrl.searchParams);if(!params.get('exclude'))params.delete('exclude');
  params.set('book',book);params.set('entity','transaction');params.set('kind','expense');params.set('state','active');
 ...
 if(resource==='transactions'){
   ...
   await withReceiptTransaction(async(c,copied)=>{for(const id of [...new Set([book,...destinations.values()])].sort())await lockBook(c,id);const created:string[]=[];
    for(const raw of [...items].sort((a,b)=>Number(a.kind==='refund')-Number(b.kind==='refund'))){
     ...
     if(raw.action==='merge'){
      ...
      const d=z.object({...details,payee:z.string().max(120).default(''),category:z.string().max(60).default('其他'),note:z.string().max(2000).default(''),externalId:z.string().max(200).optional()}).parse(raw);
      ...
      if(raw.activityId!==undefined)await setEntryActivity(c,u.id,book,raw.mergeId,z.string().uuid().nullable().parse(raw.activityId));
      ...
      const finalExternalId=old.external_id||d.externalId||null;if(finalExternalId&&(await c.query('SELECT 1 FROM transactions WHERE book_id=$1 AND external_id=$2 AND id<>$3',[book,finalExternalId,raw.mergeId])).rowCount)throw new Failure('这个交易流水号已用于另一笔账目，请核对后再保存');const finalCategory=old.category==='其他'?d.category:old.category;const result=await c.query("UPDATE transactions SET product=$1,order_id=$2,platform=$3,occurred_at=$4,note=$5,payee=$6,category=$7,line_items=$10::jsonb,external_id=$11,version=version+1 WHERE book_id=$8 AND id=$9",[old.product||d.product,old.order_id||d.orderId,old.platform||d.platform,old.occurred_at||d.occurredAt,old.note||d.note,old.payee||d.payee,finalCategory,book,raw.mergeId,JSON.stringify(old.line_items?.length?old.line_items:d.lineItems),finalExternalId]);await c.query("UPDATE transactions SET scene=$1::jsonb WHERE id=$2",[JSON.stringify({...d.scene,...Object.fromEntries(Object.entries(old.scene||{}).filter(([k,v])=>v&&!(k==='type'&&v==='general')))}),raw.mergeId]);merged+=result.rowCount||0;if(result.rowCount&&(old.kind==='expense'||old.kind==='income'))await recordCategoryFeedback(c,book,u.id,{id:raw.mergeId,category:finalCategory,categorySuggestion:d.categorySuggestion},old.category);if(old.kind==='expense')await c.query('UPDATE transactions SET category=$1,version=version+1 WHERE book_id=$2 AND refund_of=$3 AND NOT deleted',[finalCategory,book,raw.mergeId]);continue;
     }
     const e=entry.parse(raw),destination=destinations.get(raw.id)!;
     if(e.kind==='refund'&&e.refundOf&&destination!==book&&!created.includes(e.refundOf)){
      await member(destination,u,true);Object.assign(e,await copyEntryReceipts(c,book,destination,u.id,e,copied));
      added+=await insertEntry(c,destination,u.id,e);continue;
     }
     const count=await insertEntry(c,book,u.id,e);added+=count;if(count)created.push(e.id);
    }
  ...
  if(method==='PUT'){const targetBook=body.targetBook?uuid.parse(body.targetBook):book;const e=entry.parse(body);checkVerification(e.lineItems,e.amount,e.verificationReason);const version=z.number().int().parse(body.version);const result=await withReceiptTransaction(async(c,copied)=>{await lockRelatedBooks(c,book,e.id,u.id,targetBook);const previous=(await c.query("SELECT account_id,target_id,category,line_items,verification_reason,amount::float8 AS amount,kind,to_char(date,'YYYY-MM-DD') AS date FROM transactions WHERE book_id=$1 AND id=$2",[book,e.id])).rows[0];if(body.lineItems===undefined&&previous)e.lineItems=previous.line_items;if(body.verificationReason===undefined&&previous)e.verificationReason=previous.verification_reason;checkVerification(e.lineItems,e.amount,e.verificationReason);await validateActiveAccounts(c,book,e,u.id,previous?[previous.account_id,previous.target_id].filter(Boolean):[]);if(previous&&(previous.amount!==e.amount||previous.kind!==e.kind||previous.date!==e.date||previous.account_id!==e.accountId||previous.target_id!==(e.targetId||null))){if((await c.query(`SELECT 1 FROM accounts a WHERE a.id=ANY($1::uuid[]) AND NOT ${accountAccess('$2')}`,[[previous.account_id,previous.target_id].filter(Boolean),u.id])).rowCount)throw new Failure('只有资产所有者或家庭成员可以修改实际收付款信息',403);await validateActiveAccounts(c,book,e,u.id);}const refundBook=e.kind==='refund'&&e.refundOf&&targetBook!==book&&(await c.query("SELECT 1 FROM transactions WHERE id=$1 AND book_id=$2 AND kind='expense' AND NOT deleted",[e.refundOf,targetBook])).rowCount?targetBook:book;if(refundBook!==book)await member(refundBook,u,true);await validateRefund(c,refundBook,e);await checkCategory(c,u.id,e.category,previous?.category);await syncFinancialFacts(c,book,e,u.id);if(e.externalId&&(await c.query('SELECT 1 FROM transactions WHERE book_id=$1 AND external_id=$2 AND id<>$3',[book,e.externalId,e.id])).rowCount)throw new Failure('这个交易流水号已用于另一笔账目，请核对后再保存');const r=await c.query('UPDATE transactions SET account_id=$1,target_id=$2,kind=$3,amount=$4,date=$5,payee=$6,category=$7,note=$8,product=$12,platform=$13,order_id=$14,occurred_at=$15,refund_of=$16,line_items=COALESCE($17::jsonb,line_items),external_id=$18,version=version+1 WHERE id=$9 AND book_id=$10 AND version=$11 AND NOT deleted RETURNING version',[e.accountId,e.kind==='transfer'?e.targetId:null,e.kind,e.amount,e.date,e.payee,e.category,e.note,e.id,book,version,e.product,e.platform,e.orderId,e.occurredAt,e.refundOf||null,body.lineItems===undefined?null:JSON.stringify(e.lineItems),e.externalId||null]);if(!r.rowCount)throw new Failure('记录已被其他成员修改，请刷新后再试',409);await c.query('UPDATE transactions SET verification_reason=$1,title=COALESCE($3,title),scene=COALESCE($4::jsonb,scene) WHERE id=$2',[e.verificationReason,e.id,body.title===undefined?null:e.title,body.scene===undefined?null:JSON.stringify(e.scene)]);if(e.activityId!==undefined)await setEntryActivity(c,u.id,book,e.id,e.activityId);await linkReceipts(c,book,u.id,e.id,e.attachmentIds,e.retainReceipts);await linkReceipts(c,book,u.id,e.id,e.photoIds,true,'photo');if(previous?.category!==e.category&&(e.kind==='expense'||e.kind==='income'))await recordCategoryFeedback(c,book,u.id,e,previous.category);else await persistMemoryFeedback(c,{id:e.id,...(body.memorySuggestions!==undefined?{memorySuggestions:e.memorySuggestions}:{})});if(e.kind==='expense')await c.query('UPDATE transactions SET category=$1,version=version+1 WHERE book_id=$2 AND refund_of=$3 AND NOT deleted',[e.category,book,e.id]);if(targetBook!==book)await moveInTransaction(c,book,u.id,targetBook,refundBook!==book?(await c.query('SELECT * FROM transactions WHERE id=$1 AND book_id=$2',[e.id,book])).rows:await selectedGroup(c,book,[e.id]),copied);return {...r.rows[0],book:targetBook};});return NextResponse.json(result);}
