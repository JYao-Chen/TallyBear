...
export async function changeCategory(book:string,body:unknown){
 ...
 if(b.operation==='delete'){if(!old)throw new Failure('分类不存在');await c.query('INSERT INTO category_preferences(book_id,name,icon,archived,deleted) VALUES($1,$2,$3,true,true) ON CONFLICT(book_id,name) DO UPDATE SET archived=true,deleted=true',[book,b.name,old.icon]);return {ok:true};}
  ...
  await c.query('UPDATE transactions SET category=$1,version=version+1 WHERE book_id=$2 AND category=$3',[b.newName,book,b.name]);await c.query('UPDATE budgets SET category=$1 WHERE book_id=$2 AND category=$3',[b.newName,book,b.name]);await c.query('UPDATE category_feedback SET final_category=CASE WHEN final_category=$3 THEN $1 ELSE final_category END,proposed_category=CASE WHEN proposed_category=$3 THEN $1 ELSE proposed_category END,original_category=CASE WHEN original_category=$3 THEN $1 ELSE original_category END WHERE book_id=$2 AND (final_category=$3 OR proposed_category=$3 OR original_category=$3)',[b.newName,book,b.name]);
