...
export async function executeJob(job:any){
 ...
 const checkpoint={get:async(key:string)=>(await db.query('SELECT result FROM ai_job_steps WHERE job_id=$1 AND step=$2',[job.id,key])).rows[0]?.result,set:async(key:string,value:any)=>{await db.query('INSERT INTO ai_job_steps VALUES($1,$2,$3) ON CONFLICT(job_id,step) DO UPDATE SET result=$3',[job.id,key,JSON.stringify(value)]);}};
