...
export async function validateRefund(c:PoolClient,book:string,e:Entry,eventId?:string){
 ...
 if(linked.length){const dates=await c.query('SELECT id FROM transactions WHERE book_id=$1 AND refund_of=$2 AND NOT deleted AND date<$3::date',[book,e.id,e.date]);if(dates.rowCount)throw new Failure('消费日期不能晚于已关联退款');}
