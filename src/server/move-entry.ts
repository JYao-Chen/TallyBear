...
export async function moveEntry(book:string,user:string,body:unknown){
 const b=z.object({id:z.string().uuid(),targetBook:z.string().uuid(),versions:z.record(z.number().int()),accounts:z.record(z.string().uuid()).optional()}).parse(body);
 ...
 try{return await transaction(async c=>{
  ...
  for(const r of rows){await checkCategory(c,b.targetBook,r.category);
  ...
  for(const r of rows)await c.query('UPDATE transactions SET book_id=$1,version=version+1 WHERE id=$2',[b.targetBook,r.id]);
