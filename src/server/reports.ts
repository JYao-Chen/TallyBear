...
export function reportFilter(book:string,params:URLSearchParams){
 ...
 const values:unknown[]=[book,input.from,input.to];let where='t.book_id=$1 AND NOT t.deleted AND t.date BETWEEN $2::date AND $3::date';
 ...
 if(input.q){values.push(input.q);where+=' AND '+textMatch("ARRAY[t.title,t.payee,t.product,t.note,t.category,t.platform,a.name,t.order_id,t.external_id]||ARRAY(SELECT i->>'name' FROM jsonb_array_elements(t.line_items) i)",'$'+values.length,input.mode);}
 return {input,values,where};
}
const projection='t.title,t.id,t.event_id,t.account_id,t.target_id,t.kind,t.amount::float8 AS amount,t.date,t.payee,t.category,t.note,t.external_id,t.created_at,t.version,t.verification_reason,t.line_items,t.product,t.platform,t.order_id,t.occurred_at,t.refund_of,a.name AS account_name,a.type AS account_type,a.holder AS account_holder,a.institution AS account_institution,a.suffix AS account_suffix,a.ownership AS account_ownership,(SELECT row_to_json(dest) FROM accounts dest WHERE dest.id=t.target_id) AS target_account,(SELECT name FROM accounts dest WHERE dest.id=t.target_id) AS target_name,u.name AS creator_name';
const joins='transactions t JOIN accounts a ON a.id=t.account_id JOIN users u ON u.id=t.created_by';
export async function report(book:string,params:URLSearchParams){
 const {input,values,where}=reportFilter(book,params);values.push(input.limit,input.offset);
 const result=await db.query(`WITH filtered AS MATERIALIZED (SELECT ${projection} FROM ${joins} WHERE ${where}), page AS (SELECT * FROM filtered ORDER BY date DESC,created_at DESC,id DESC LIMIT $${values.length-1} OFFSET $${values.length}), categories AS (SELECT category AS name,sum(CASE WHEN kind='refund' THEN -amount ELSE amount END)::float8 AS value FROM filtered WHERE kind IN ('expense','refund') GROUP BY category), daily AS (SELECT to_char(date,'YYYY-MM-DD') AS date,COALESCE(sum(amount) FILTER(WHERE kind='income'),0)::float8 AS income,COALESCE(sum(CASE WHEN kind='refund' THEN -amount ELSE amount END) FILTER(WHERE kind IN ('expense','refund')),0)::float8 AS expense FROM filtered GROUP BY date) SELECT json_build_object('rows',(SELECT COALESCE(json_agg(page),'[]'::json) FROM page),'totals',(SELECT json_build_object('count',count(*),'income',COALESCE(sum(amount) FILTER(WHERE kind='income'),0),'expense',COALESCE(sum(amount) FILTER(WHERE kind='expense'),0),'refund',COALESCE(sum(amount) FILTER(WHERE kind='refund'),0)) FROM filtered),'categories',(SELECT COALESCE(json_agg(categories ORDER BY value DESC),'[]'::json) FROM categories),'daily',(SELECT COALESCE(json_agg(daily ORDER BY date),'[]'::json) FROM daily)) AS result`,values);
 return result.rows[0].result;
}
export async function exportCSV(book:string,params:URLSearchParams,locale:Language='zh-CN'){
 const {values,where}=reportFilter(book,params);const rows=(await db.query(`SELECT ${projection},to_char(t.date,'YYYY-MM-DD') AS date FROM ${joins} WHERE ${where} ORDER BY t.date DESC,t.created_at DESC,t.id DESC`,values)).rows;
