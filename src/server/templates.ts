...
export async function templates(book:string,user:string,method:string,body:unknown){
 ...
 if(method==='POST'||method==='PUT'){
  if(!b.name)throw new Failure('请给常用记录起个名字');const item=entry.parse(b.value);if(item.kind==='refund')throw new Failure('退款需要核对原消费，请单独记录');const ids=[item.accountId,...(item.kind==='transfer'?[item.targetId]:[])];const valid=(await db.query('SELECT id FROM accounts WHERE book_id=$1 AND id=ANY($2::uuid[]) AND NOT archived',[book,ids])).rows;if(valid.length!==ids.length)throw new Failure('请选择当前账本中可用的资金账户');
