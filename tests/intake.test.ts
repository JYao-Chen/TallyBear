...
test('缺失金额账户必须补充，空日期使用记录日期',()=>{assert.deepEqual(missingFields({...a,amount:0,date:'',accountId:''}),['金额','账户']);});
test('部分退款可关联早前订单，返现超额仍可关联，不匹配未来消费',()=>{assert.equal(refundCandidates({...a,id:'refund',kind:'refund',amount:500,date:'2026-10-01'},[a]).length,1);assert.equal(refundCandidates({...a,id:'refund',kind:'refund',amount:2000},[a]).length,1);});

test('distinct order references keep repeat purchases separate',()=>{assert.equal(matchReason(a,{...a,id:'b',orderId:'order2'}),null);});
