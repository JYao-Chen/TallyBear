...
test('nine images batch automatically and complementary orders merge without adding payment twice',async()=>{
 ...
 try{
  ...
  assert.deepEqual(batches,[6,4]);assert.equal(merges,1);assert.equal(result.entries.length,2);assert.equal(result.entries[0].amount,1800);assert.ok(result.entries[0].source.includes('原图9'));assert.equal(result.ignored.length,1);assert.deepEqual(result.entries[1].missing,['金额','日期','账户']);
